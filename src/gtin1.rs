...
fn get_n_ints(n: i32) -> Result<Vec<i32>, &'static str> {
    ...
    let ints: Vec<i32> = number.trim().chars().map(|s| s.to_digit(10).unwrap() as i32).collect();
...
pub fn main() {
    ...
    if let Some('c') = choice {
        ...
        println!("This code is {}valid.", if calc_checksum(get_n_ints(8).unwrap()) != 0 {"not "} else {""})
    } else {
